...
data = dict(
    ...
    width = WIDTH,
    frame_ids = FRAME_IDS,
    in_path = './datasets/kitti_data',#path to kitti raw data
    gt_depth_path = './splits/eigen/gt_depths.npz',#path to kitti depth ground truth
    png = False,
...
model = dict(
    ...
    min_depth = 0.1,
    max_depth = 100.0,
    depth_pretrained_path = './weights/resnet{}.pth'.format(DEPTH_LAYERS),#path to pre-trained resnet weights
    pose_pretrained_path =  './weights/resnet{}.pth'.format(POSE_LAYERS),#path to pre-trained resnet weights
...
resume_from = './weights/fm_depth.pth'#we will resume from current epoch for further online refinement
