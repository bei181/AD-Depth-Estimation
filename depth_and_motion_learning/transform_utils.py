...
def invert_rot_and_trans(rot, trans):
  ...
  inv_trans = -tf_matmul_separate(inv_rot_mat, tf.expand_dims(trans, -1)) # -tf.matmul(inv_rot_mat, tf.expand_dims(trans, -1))
...
def combine(rot_mat1, trans_vec1, rot_mat2, trans_vec2):
  ...
  r2r1 = tf_matmul_separate(rot_mat2, rot_mat1)  # tf.matmul(rot_mat2, rot_mat1)
  r2t1 = tf_matmul_separate(rot_mat2, tf.expand_dims(trans_vec1, -1))  # tf.matmul(rot_mat2, tf.expand_dims(trans_vec1, -1))
