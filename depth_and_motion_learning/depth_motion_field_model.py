...
import depth_prediction_nets
import intrinsics_utils
import maybe_summary
import object_motion_nets
import parameter_container
import transform_utils
from dataset import data_processing
from dataset import reader_cityscapes, reader_mydata
from losses import loss_aggregator
from transform_utils import tf_matmul_separate
...
def input_fn(params):
  ...
  if params.input.reader != None:
        reader = reader_mydata
  else:
        reader = reader_cityscapes

  params.input.reader=None
  dataset = reader.read_frame_pairs_from_data_path(
...
def infer_egomotion(rgb_image1, rgb_image2, params):
  ...
  avg_trans = 0.5 * (-tf.squeeze(
      tf_matmul_separate(rot12, tf.expand_dims(trans21, -1)), axis=-1) + trans12)
