...
HEIGHT = 128
SUB_FOLDER = 'train'
INPUT_DIR = '/home/tata/Project/dataset/Cityscapes'
OUTPUT_DIR = '/home/tata/Project/dataset/cityscapes_processed'
  ...
  for location in glob.glob(INPUT_DIR + '/leftImg8bit_sequence/' + SUB_FOLDER + '/*'):
    ...
    for (k,v) in sequences.items():
        ...
        for j in range(0, len(files), SKIP):
            ...
            smallimg, fx_this, fy_this, cx_this, cy_this = crop(img, fx, fy, cx, cy)
...
if os.path.exists(OUTPUT_DIR + '/' + SUB_FOLDER + '.txt'):
    os.remove(OUTPUT_DIR + '/' + SUB_FOLDER + '.txt')
    
fn = open(OUTPUT_DIR + '/' + SUB_FOLDER + '.txt', 'x')
