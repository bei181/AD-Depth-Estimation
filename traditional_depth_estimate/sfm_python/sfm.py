...
def extract_features(image_names):
    ...
    for image_name in image_names:
        ...
        if len(key_points) <= 10:
            continue
        images_for_all.append(image)
        imagenames_for_all.append(image_name)
    ...
    return np.array(key_points_for_all), np.array(descriptor_for_all), np.array(colors_for_all), images_for_all, imagenames_for_all
...
def bundle_adjustment(rotations, motions, K, correspond_struct_idx, key_points_for_all, structure):
    ...
    for i in range(len(correspond_struct_idx)):
        ...
        for j in range(len(point3d_ids)):
            ...
            new_point = get_3dpos(structure[point3d_id], key_points[j].pt, r, t, K)
...
def fig_v3d(structure, colors):
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(structure)
    colors /= 255.0
    pcd.colors = o3d.utility.Vector3dVector(colors)
    pcd_filter, ind = pcd.remove_radius_outlier(nb_points=3, radius=1)
    o3d.visualization.draw_geometries([pcd_filter])

def draw_points(points, img, depth):
    depth = np.asarray(depth)
    depth = depth[:, np.newaxis]
    print(depth.shape)
    points = np.asarray(points)
    # img[points[:, 1], points[:, 0]] = depth
    normalizer = mpl.colors.Normalize(vmin=depth.min(), vmax=depth.max())
    print(depth.min(), depth.max())
    mapper = cm.ScalarMappable(norm=normalizer, cmap='magma')
    for ii, point in enumerate(points):
        # print(type(depth[ii]))
        color = (mapper.to_rgba(depth[ii]) * 255)[0,:3]
        color = (int(color[0]), int(color[1]), int(color[2]))
        cv2.circle(img, (int(point[0]), int(point[1])), 4, color=color, thickness=-1)

    return img

def point3D_to_point2D(point3D, K, R, t, image, image_name):
    ...
    def project(X):
        ...
        x[1, :] /= x[2, :]
        return x[:2, :], x[2, :]
    point2D, depth = project(point3D)
    
    point2D = point2D.transpose()

    # 超出图像范围的点丢掉
    cam2pixel_points = list()
    cam2pixel_depth = list()
    for i in range(point2D.shape[0]):
        x, y = point2D[i]
        d = depth[i]
        if x > 0 and x < width and y > 0 and y < height:
            cam2pixel_points.append((int(x),int(y)))
            cam2pixel_depth.append(d) 

    img = draw_points(cam2pixel_points, img, cam2pixel_depth)
    save_path = image_name.replace("/home/swei/ztworks/ailab/python/data/Herz-Jesus-P8/images", "/home/swei/ztworks/ailab/python/Sfm-python/results")
...
def main():
    ...
    key_points_for_all, descriptor_for_all, colors_for_all, images_for_all, imagenames_for_all = extract_features(img_names)
    ...
    for i in range(1, len(matches_for_all)):
        ...
        #2D 深度图
        point3D_to_point2D(next_structure, K, R, T, images_for_all[i], imagenames_for_all[i])
        #单幅重建图
        #fig_v3d(next_structure, c1)
